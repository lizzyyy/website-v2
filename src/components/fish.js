...
import sakura from "../images/goldfish.gif";
...
class Fish extends Component {
  ...
  constructor() {
    ...
    this.state = {
      x: Math.random() * 
        (window.innerWidth * 0.1 - Constant.max_scale_factor * Constant.image_width),
      xDirection: 'right',
      xVelocity: 2,

      y: Math.random() * 
        (window.innerHeight * 0.4 - Constant.max_scale_factor * Constant.image_height),
      yDirection: 'down',
      yVelocity: 1,
  ...
  relocate() {
    this.setState({
    x: Math.random() * 
      (window.innerWidth * 0.3 - Constant.max_scale_factor * Constant.image_width),

    y: Math.random() * 
      (window.innerHeight * 0.5 - Constant.max_scale_factor * Constant.image_height),
    yDirection: 'down',
    yVelocity: 1,
  ...
  move() {
    ...
    if (this.state.x > window.innerWidth * 0.32  - window.innerWidth * 0.1|| this.state.y > window.innerHeight * 0.3) {
      this.relocate(); // if the fish is outisde the window (window was resized, probably)
    } 

    if (this.state.x > 
      (window.innerWidth * 0.15 - Constant.max_scale_factor * Constant.image_width)) {
      xDirection = 'left';
    } else if (this.state.x < Constant.max_scale_factor * Constant.image_width - window.innerWidth * 0.03) {
      xDirection = 'right';
    }
  ...
  render() {
    ...
    return (
      <img className={s.fish} style={fishStyle} src={sakura}/>
    );
  }
